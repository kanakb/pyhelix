...
class HelixTask(object):
    ...
    def call(self):
        """
        Process the message. Calls application callbacks and then updates the global state.
        """
        thread = threading.current_thread()
        logging.info('{0} invokes message: {1}'.format(str(thread), self._message))
        from_state = self._message['simpleFields']['FROM_STATE']
        to_state = self._message['simpleFields']['TO_STATE']
        method_to_invoke = self._state_model_parser.get_method_for_transition(self._state_model, from_state, to_state)
        ...
        if to_state != 'DROPPED':
            ...
            current_state['mapFields'][partition_name]['CURRENT_STATE'] = to_state
            current_state['simpleFields']['STATE_MODEL_DEF'] = self._message['simpleFields']['STATE_MODEL_DEF']
            current_state['simpleFields']['SESSION_ID'] = self._participant.get_session_id()
        else:
            # drop the partition from the current state
            sub = True
        self._accessor.update(self._builder.current_state(self._participant_id,
            self._participant.get_session_id(), resource_name), current_state, sub=sub)
        self._accessor.remove(self._builder.message(self._participant_id, self._message['id']))
